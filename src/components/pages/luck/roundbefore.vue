...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      //     this.isClick = true //如果为0则不能点击
      //   }
      //   for (var i = 0; i < res.data.prizes.length; i++) {
      //     if (res.data.prizes[i].prizeName.length > 5) {
      //       res.data.prizes[i].prizeName = res.data.prizes[i].prizeName.substring(
      //         0,
      //         6
      //       );
      //     }
    ...
    handleClick() {
      ...
      //   let mathrandom = this.extraDeg[this.degIndex]
      this.rotateDeg = "rotate(" + (360 * 8 + parseInt(59)) + "deg)";
      setTimeout(() => {
        this.isModel = true;
      }, 3500);
      ...
      // };
      //     this.$axios
      //       .post("/activityCenter/userApp/raffle/obtain/prize", postData)
      //       .then(res => {
      //         if (res.success) {
      ...
      //           this.rotateDeg =
      //             "rotate(" +
      //             (360 * 8 + parseInt(this.extraDeg[this.degIndex])) +
      //             "deg)";
      //           setTimeout(() => {
      //             this.isModel = true;
      //           }, 3500);
